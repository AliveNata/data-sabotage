...
function getPlayerView(room: GameRoom, playerId: string) {
  ...
  for (const [id, p] of Object.entries(room.players)) {
    players[id] = {
      ...
      isAlive: p.isAlive,
      isGod: p.isGod,
      roleId: isGod || id === playerId || room.phase === 'gameover' ? p.roleId : null,
...
app.prepare().then(() => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('game:start', () => {
      ...
      if (result) { broadcastRoom(result); broadcastRoomList(); }
    });
  ...
  httpServer.listen(port, () => {
    console.log(`> Data Sabotage running on http://localhost:${port}`);
  });
});
